...
namespace FitForge.DL
{
    public static class DB
    {
        ...
        // Runs several statements as ONE atomic unit: either they all commit, or (on any
        // exception) they all roll back, leaving no partial/orphaned rows behind. Pass a
        // delegate that takes the open connection + transaction and issues its own commands
        // against them (so everything shares one connection instead of DB.NonQuery's usual
        // one-connection-per-call pattern).
        public static T Transaction<T>(Func<MySqlConnection, MySqlTransaction, T> work)
        {
            using var c = Conn();
            c.Open();
