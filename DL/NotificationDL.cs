...
namespace FitForge.DL
{
    ...
    public class NotificationDL(ILogger<NotificationDL> log)
    {
        ...
        // Time-of-day matching happens in C# now (NotificationBL), not here — MySQL has no
        // reliable IANA-timezone conversion without the mysql.time_zone_name tables loaded
        // (many managed hosts, including Aiven by default, don't have them), whereas .NET's
        // TimeZoneInfo handles IANA ids natively on Linux. So this just returns every
        // still-eligible candidate for today (right weekday, not already reminded today,
        // not already finished if it's a workout day) — the caller filters by whether each
        // user's own local clock has actually reached their chosen reminder_time yet.
        public List<ReminderCandidate> GetWorkoutReminderCandidates(int weekDay)=>
            DB.Select(@"SELECT DISTINCT u.user_id,ss.day_id,pd.name AS day_name,
              ...
              .Rows().Select(r=>new ReminderCandidate{
                  ...
                  ReminderTime=r["reminder_time"] is TimeSpan rt?rt:TimeSpan.Parse(r["reminder_time"].ToString()!),
                  Timezone=r["timezone"]?.ToString()??"UTC"}).ToList();
